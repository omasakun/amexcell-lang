...
export function proto2ins(sheet: Sheet, map: Map<string, ModuleProto>): Module {
	...
	return makeModule(sheet, map, "main");
}
...
function makeModule(sheet: Sheet, map: Map<string, ModuleProto>, mName: string): Module {
	const proto = map.get(mName);
	if (!proto) throw `定義されていないモジュールの実体を作ろうとしています。Name: ${mName}。`;
	const result: Module = { in: new Map(), out: new Map(), var: new Map(), modules: new Map(), eqs: [], moduleName: mName };
	proto.in.forEach(v => result.in.set(v.name, sheet.alloc(v.size, mName + ".in." + v.name)));
	proto.out.forEach(v => result.out.set(v.name, sheet.alloc(v.size, mName + ".out." + v.name)));
	proto.var.forEach(v => result.var.set(v.name, sheet.alloc(v.size, mName + ".var." + v.name)));
	proto.modules.forEach((v, k) => result.modules.set(k, makeModule(sheet, map, v)));
